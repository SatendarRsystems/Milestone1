...
export class VidyoClientService {
    ...
    */
    ...
    /**
    * Description: Set user name and meeting room variable.
    ...
    */
    generateToken = () => {
        ...
        const expires = Math.floor(Date.now() / 1000) + config.EXPIRE_TIME + EPOCH_SECONDS;
        const shaObj = new jsSHA('SHA-384', 'TEXT');
        shaObj.setHMACKey(config.API_KEY, 'TEXT');
        const jid = this.userName + '@' + config.APP_ID;
